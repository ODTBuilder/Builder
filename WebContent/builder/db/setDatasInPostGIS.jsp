...
try{
	...
	/* --------------- 여기서부터 DB 연결하고 Insert 하는 부분 ---------------- */
	
	if(feature_type.equals("Point")){
		String spatialQuery = "";
		
		spatialQuery = "insert into tst_table_point(name, the_geom)" ;
		spatialQuery += "values(" + "'테스트', ST_GeomFromText('POINT(" ;
		for(int i=0 ; i<array.length() ; i++){
			if(i+1 == array.length()){
		...
		System.out.println("성공개수 = " + success_count);
	}
	...
	if(feature_type.equals("Line")){
		...
		spatialQuery = "insert into tst_table_line(name, the_geom)" ;
		spatialQuery += "values(" + "'테스트', ST_GeomFromText('LINESTRING(";
		for(int i=0 ; i<array.length() ; i++){
			if(i+1 == array.length()){
				System.out.println("i+1 == array.length");
				spatialQuery += geo_x[i].toString() + " " + geo_y[i].toString();
	...
	if(feature_type.equals("Polygon")){
		...
		spatialQuery = "insert into tst_table(name, the_geom)" ;
		spatialQuery += "values(" + "'테스트', ST_GeomFromText('MULTIPOLYGON(((";
		for(int i=0 ; i<array.length() ; i++){
			spatialQuery += geo_x[i].toString() + " " + geo_y[i].toString() + ",";
		}
		spatialQuery += geo_x[0].toString() + " " + geo_y[0].toString();
