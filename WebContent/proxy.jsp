<%@page session="false"%>
<%@page import="java.net.*,java.io.*"  contentType="text/xml; charset=UTF-8"%>
 ...
 */
String[] allowedHosts = {
  "175.116.181.22:8090", "localhost:8090"
};

HttpURLConnection con = null;
try {
	//out.println("HttpRequest has started.");
	...
	for (String surl : allowedHosts) {
		if (host.equalsIgnoreCase(surl)) {
			allowed = true;
			//out.println("Proxy.jsp : is allowed irl , " + surl	);
			break;
		}
		else
		{
			//out.println("Proxy.jsp : is not allowed irl , " + surl	);
		}
	}
	...
	if(allowed) {
		...
		//out.println("URL : " + reqUrl);
		// call the requested ressource		
		URL url = new URL(reqUrl);
		out.println("<url " + url.toString() + "></url>");
